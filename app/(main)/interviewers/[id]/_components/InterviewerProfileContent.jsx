...
const InterviewerProfileContent = ({ interviewer, userCredits, userRole }) => {
  const [upgradeOpen, setUpgradeOpen] = React.useState(false);
  const { has } = useAuth();

  const getExpectItems = () => {
    return EXPECT_ITEMS.map(([icon, title, desc]) => {
      // Interviewers have full access to everything
      if (userRole === "INTERVIEWER") {
        return [icon, title, desc, null];
      }
      
      // Apply restrictions only to interviewees
      if (title === "AI Feedback Report") {
        const hasAccess = has?.({ plan: "starter" }) || has?.({ plan: "pro" });
        return [
          icon,
          title,
          desc,
          hasAccess ? null : "🔒 Starter plan"
        ];
      }
      if (title === "Recording & Playback") {
        const hasAccess = has?.({ plan: "pro" });
        return [
          icon,
          title,
          desc,
          hasAccess ? null : "🔒 Pro plan"
        ];
      }
      return [icon, title, desc, null];
    });
  ...
  return (
    <>
      <UpgradeModal
        ...
        reason="Upgrade your plan to access premium features like AI feedback and session recordings."
      />
        ...
        <div className="lg:col-span-3 flex flex-col gap-6 order-2 lg:-order-1">
          ...
          <div className="bg-[#0f0f11] border border-white/10 rounded-2xl p-8 flex flex-col gap-6">
            ...
            <ul className="flex flex-col gap-5">
              {getExpectItems().map(([icon, title, desc, lockText], index) => (
...
export default InterviewerProfileContent;
